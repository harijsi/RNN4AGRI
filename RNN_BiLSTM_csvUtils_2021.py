...
param_grid = {'-learning_rate': [1e-2, 1e-3], '-batch_size': [64, 128, 256],
...
parser.add_argument('-sequence_name', default=f'RNN_LSTM_CSV_2021_310721', type=str)
parser.add_argument('-learning_rate', default=1e-3, type=float)
parser.add_argument('-batch_size', default=128, type=int)
parser.add_argument('-epochs', default=1500, type=int)
parser.add_argument('-is_cuda', default=True, type=lambda x: (str(x).lower() == 'true'))
parser.add_argument('-sequence_len', default=12, type=int)
...
class ModelBiLSTM(torch.nn.Module):
    ...
    def forward(self, x_packed):
        ...
        x_packed = torch.nn.utils.rnn.pack_padded_sequence(x_packed, lengths, batch_first=True, enforce_sorted=True)
...
for epoch in range(0, args.epochs):
    ...
    for data_loader in [data_loader_train, data_loader_test]:
        ...
        for x, y in data_loader:
            ...
            loss = -torch.mean(torch.log((y_prim*class_weights)[idxes, y[idxes]] + 1e-20))
